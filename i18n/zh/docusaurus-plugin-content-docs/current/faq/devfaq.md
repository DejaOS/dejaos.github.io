...
1.  **启动屏幕背景可以更换，但无法通过普通应用代码动态修改。**
    联系我们获取**对应机型的升级应用**，在应用中补充您想要的图片，然后在设备上执行该应用即可。更多详情请参考：[更便捷地更换设备启动背景](/changelog/change-notes/2026#07-20)。

2.  **不同设备的升级应用不一样。**
    需由我们按机型单独提供。该功能后续会集成到系统管理应用中，**暂时不会发布到 GitHub**。

3.  **图片分辨率必须与设备屏幕一致。**
    各机型分辨率可在 `dxDriver.js` 中查阅。

4.  **出厂预装**不是少量订单的默认服务。若有**大批量**设备，可提供预装服务；否则可能需要额外收费，请联系我们确认。
