...
2. 点击 VSCode 底部的 `install` 按钮下载依赖的库
3. 连接后，点击 `syncAll` 进行初始完整同步（这可能需要一些时间）
...
如果您偏好终端开发、自动化脚本或 AI 辅助编程，也可以安装 [DejaOS CLI](./cli.md)：
...
后续修改代码后，可以直接执行 `dejaos run`，自动完成停止应用、增量同步和重新启动。
...
使用命令行时，可以执行 `dejaos run` 同步并运行项目，执行 `dejaos logs` 持续查看设备实时日志。
